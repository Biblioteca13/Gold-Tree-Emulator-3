...
namespace GoldTree.HabboHotel.Support
{
	internal sealed class ModerationTool
	{
		...
		public void method_8(GameClient Session, uint uint_0, int int_0)
		{
			...
			if (@class != null && @class.Status == TicketStatus.PICKED && @class.ModeratorId == Session.GetHabbo().Id)
			{
				GameClient class2 = GoldTree.GetGame().GetClientManager().method_2(@class.SenderId);
				int int_;
				TicketStatus enum6_;
				switch (int_0)
				{
		...
		{
			Room @class = GoldTree.GetGame().GetRoomManager().GetRoom(uint_0);
			if (@class != null)
			{
				if (bool_1)
				{
		...
		public ServerMessage method_19(uint uint_0)
		{
			...
			using (DatabaseClient @class = GoldTree.GetDatabase().GetClient())
			{
				...
				Message.AppendStringWithBreak(GoldTree.GetGame().GetClientManager().GetNameById(uint_0));
				if (dataTable != null)
				{
					...
					try
					{
						while (enumerator.MoveNext())
						{
							...
							RoomData class2 = GoldTree.GetGame().GetRoomManager().method_11((uint)dataRow["room_id"]);
